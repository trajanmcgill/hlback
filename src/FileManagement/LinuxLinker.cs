...
namespace hlback.FileManagement
{
	...
	class LinuxLinker : ILinker
	{
		...
		{
			// In Linux, need to do this by kicking off the ln command.
			// Set up the info for starting an external process to run ln and pass it the appropriate arguments.
			...
			// Create the ln command process and wait for it to complete before returning.
